...
public class PumpDetailsActivity extends Activity {
    ...
    protected void onCreate(Bundle b) {
        ...
        TextView model = Ui.text(this, rec.model, 28, Ui.TEXT, 1);
        model.setPadding(Ui.dp(this, 10), 0, 0, 0);
        row.addView(model);
        top.addView(row);
        top.addView(Ui.text(this, rec.category, 15, Ui.BLUE, 1));
        top.addView(Ui.text(this, PumpSelector.trim(rec.hp)+" HP • "+PumpSelector.trim(rec.kw)+" kW • "+phaseLabel(rec.phase), 15, Ui.MUTED, 1));
        if (hasEstimate && !Double.isNaN(head) && !Double.isNaN(flow)) {
            top.addView(Ui.text(this, "Estimated at selected head: " + PumpSelector.lph(flow) + " at " + PumpSelector.head(head), 16, Ui.GREEN, 1));
        }
        root.addView(top);

        LinearLayout chartCard = Ui.card(this);
        chartCard.addView(Ui.text(this, "Performance curve", 20, Ui.TEXT, 1));
        TextView note = Ui.text(this, "One complete end-to-end curve line for this model. The orange point is the selected operating point.", 13, Ui.MUTED, 0);
        Ui.mb(this, note, 8);
        ...
        Button copy = Ui.secondary(this, "Copy model");
    ...
    LinearLayout detailCard(String title, String[][] rows) {
        ...
        for (String[] row : rows) {
            ...
            TextView l = Ui.text(this, row[0], 13, Ui.MUTED, 1);
            TextView r = Ui.text(this, row[1], 14, Ui.TEXT, 0);
    ...
    String phaseLabel(String p) { p=safe(p).toUpperCase(Locale.US); if(p.contains("S"))return "Single Phase"; if(p.contains("T"))return "Three Phase"; return p.isEmpty()?"Phase -":p; }
    String flowRange(PumpRecord r){return Double.isNaN(r.minFlowLPH)||Double.isNaN(r.maxFlowLPH)?"-":PumpSelector.lph(r.minFlowLPH).replace(" LPH","")+" – "+PumpSelector.lph(r.maxFlowLPH);}
}
