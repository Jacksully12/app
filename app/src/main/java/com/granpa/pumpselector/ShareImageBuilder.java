...
public class ShareImageBuilder {
    public static Bitmap build(Activity a, PumpRecord r, boolean has, double head, double flow) {
        ...
        text.setTypeface(Typeface.create(Typeface.DEFAULT, Typeface.BOLD));
        text.setTextSize(30);
        drawFit(c, has ? "Suitable for " + PumpSelector.head(head) + " head  •  Estimated " + PumpSelector.lph(flow) : "Model details and smooth performance curve", 115, y + 45, text, 860);
        ...
        c.drawText(has ? PumpSelector.lph(flow) : flowRange(r), 960, y + 60, text);
        text.setTextAlign(Paint.Align.LEFT);

        drawChart(c, r, has, head, flow, new RectF(70, 790, 1010, 1320));
    ...
    static void drawChip(Canvas c, String top, String bottom, int x, int y, int w, int h) {
        ...
        for (int i = 0; i < lines.length && i < 2; i++) c.drawText(lines[i], x + w / 2f, sy + i * 28, t);
    }
    ...
    static void drawChart(Canvas c, PumpRecord r, boolean has, double head, double flow, RectF outer) {
        Paint p = new Paint(1);
        p.setStyle(Paint.Style.FILL);
        p.setColor(Color.WHITE);
        ...
        if (r.curve != null) {
            for (double[] q : r.curve) if (q != null && q.length >= 2) real.add(new double[]{q[0], q[1]});
        }
        Collections.sort(real, Comparator.comparingDouble(a -> a[1]));
        if (real.isEmpty()) return;

        ArrayList<double[]> pts = displayPoints(real, has ? head : null, has ? flow : null);
        ...
        for (int i = 0; i <= 5; i++) {
            ...
            c.drawText(String.format(Locale.US, "%,.0f", axisF * i / 5), gx, plot.bottom + 34, text);
        }
        text.setTextAlign(Paint.Align.RIGHT);
        for (int i = 0; i <= 5; i++) {
    ...
    static double niceFlowStep(double m) { if (m <= 1000) return 200; if (m <= 3000) return 500; if (m <= 10000) return 1000; if (m <= 30000) return 5000; return 10000; }
    ...
    static String phaseLabel(String p) { p = safe(p).toUpperCase(Locale.US); if (p.contains("S")) return "Single"; if (p.contains("T")) return "Three"; return p.isEmpty() ? "-" : p; }
    static String compactType(String c) { c = safe(c).toLowerCase(Locale.US); if (c.contains("borewell")) return "Borewell\nSubmersible"; if (c.contains("monoblock")) return "Monoblock"; if (c.contains("dewatering") || c.contains("sewage")) return "Dewatering"; return safe(c).replace(" - ", "\n"); }
